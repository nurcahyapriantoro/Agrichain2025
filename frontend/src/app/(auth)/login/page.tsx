...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    // Clean the URL by removing the error param
    if (errorParam) {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      console.log('Attempting login with credentials...', { email: data.email });
      ...
      if (result?.error) {
        ...
        if (result.error === 'CredentialsSignin') {
          setPasswordError('Password salah. Mohon periksa kembali dan coba lagi.');
          setError('Email atau password tidak valid. Mohon periksa kembali dan coba lagi.');
          
          // Coba login langsung ke API sebagai fallback
          try {
            ...
            console.log('Direct API login result:', apiData);
            ...
            if (apiData.success) {
              ...
              if (apiData.data?.token) {
                ...
                userData = apiData.data.user || apiData.data;
              } else if (apiData.token) {
                token = apiData.token;
                userData = apiData.user || apiData.data;
              }
              
              if (token) {
                console.log('Saving fallback auth token and user data');
              ...
              console.error('Direct API login failed:', apiData.message);
            }
          } catch (apiError) {
            console.error('Direct API login failed:', apiError);
          }
        } else {
      ...
      if (result?.ok) {
        ...
        console.error('Login result unclear:', result);
  ...
  const handleMetaMaskLogin = async () => {
    ...
    try {
      ...
      const message = `Login to AgriChain with address: ${address}`;
      const signature = await window.ethereum.request({
        method: 'personal_sign',
        params: [message, address]
      });
      
      console.log('Attempting login with MetaMask...', { address, chainId, signature });
      ...
      if (error.code === 4001) {
        ...
        setError('You rejected the signature request. Please approve it to login.');
      } else {
        setError(`Failed to login with MetaMask: ${error.message}`);
      }
    } finally {
  ...
  // Add this function to handle MetaMask login with a specific address
  const handleMetaMaskLoginWithAddress = async (address: string) => {
    try {
      setMetamaskLoading(true);
      setError(null);
      ...
      console.log('Auto-login with MetaMask...', { address, chainId, signature });
      ...
      console.error('MetaMask auto-login error:', error);
      // Silent fail for auto-login, don't show errors to the user
      // We'll just let them log in manually
    } finally {
      setMetamaskLoading(false);
    }
  };
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center px-4 sm:px-6 lg:px-8 py-12 z-10">
          <div className="sm:mx-auto sm:w-full sm:max-w-md">
            ...
            <p className="mt-2 text-center text-sm text-gray-400">
              Enter your credentials to access the blockchain platform
            </p>
          </div>
            ...
            <div className="backdrop-blur-md bg-gray-900/40 py-8 px-4 shadow-lg border border-gray-800 rounded-xl sm:px-10 
                ...
                <div>
                  ...
                  <div className="mt-1 relative group">
                    ...
                    <div className="absolute inset-y-0 right-0 pr-3 flex items-center opacity-0 group-focus-within:opacity-100 transition-opacity">
                      ...
                      <div className="h-1 w-1 rounded-full bg-green-500 animate-pulse ml-1" style={{ animationDelay: '0.2s' }}></div>
                      <div className="h-1 w-1 rounded-full bg-purple-500 animate-pulse ml-1" style={{ animationDelay: '0.4s' }}></div>
                    </div>
                  </div>
                  {errors.email && (
                ...
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-sm">
                      <Link
                        ...
                        className="font-medium bg-gradient-to-r from-purple-400 to-green-400 hover:from-purple-500 hover:to-green-500 bg-clip-text text-transparent hover:bg-clip-text transition duration-200"
                      >
                  ...
                  <div className="relative mt-1 group">
                    ...
                    <input
                      ...
                      {...register('password')}
                      className={`pl-10 block w-full rounded-lg border ${passwordError ? 'border-red-500' : 'border-gray-700'} 
                              bg-gray-800/50 shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 
                              focus:outline-none hover:border-purple-700 transition-all duration-200 sm:text-sm text-white
                              backdrop-blur-md`}
                    />
                    <div className="absolute inset-y-0 right-0 pr-3 flex items-center opacity-0 group-focus-within:opacity-100 transition-opacity">
                      <div className="h-1 w-1 rounded-full bg-purple-500 animate-pulse"></div>
              ...
              <div className="mt-6">
                ...
                <div className="mt-6 space-y-4">
                  {/* MetaMask Login Button with improved logo */}
                  <Button
                    type="button"
                    variant="outline"
